...
void solve() {
    int numMountains;
    if (!(cin >> numMountains)) return;

    vll mountainHeights(numMountains);
    forn(i, numMountains) cin >> mountainHeights[i];

    vi leftGreater(numMountains, -1);
    vi rightGreater(numMountains, -1);
    stack<int> monotonicStack;

    forn(i, numMountains) {
        while (!monotonicStack.empty() && mountainHeights[monotonicStack.top()] <= mountainHeights[i]) {
            monotonicStack.pop();
        }
        if (!monotonicStack.empty()) leftGreater[i] = monotonicStack.top();
    ...
    ford(i, numMountains) {
        ...
        if (!monotonicStack.empty()) rightGreater[i] = monotonicStack.top();
        monotonicStack.push(i);
    }

    vi sortedIndices(numMountains);
    iota(all(sortedIndices), 0);
    sort(all(sortedIndices), [&](int a, int b) {
        if (mountainHeights[a] != mountainHeights[b]) return mountainHeights[a] > mountainHeights[b];
        return a < b;
    });

    vll maxMountainsDp(numMountains, 0);
    ll maxVisited = 0;

    for (int i : sortedIndices) {
        ll bestParentDp = 0;
        if (leftGreater[i] != -1) {
            bestParentDp = max(bestParentDp, maxMountainsDp[leftGreater[i]]);
        }
        if (rightGreater[i] != -1) {
            bestParentDp = max(bestParentDp, maxMountainsDp[rightGreater[i]]);
        }
        maxMountainsDp[i] = 1 + bestParentDp;
        maxVisited = max(maxVisited, maxMountainsDp[i]);
    }
