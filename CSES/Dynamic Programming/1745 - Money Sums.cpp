...
void solve() {
    ...
    vi coinValues(numCoins);

    forn(i, numCoins) cin >> coinValues[i];

    bitset<100001> possibleSums;
    possibleSums[0] = 1;

    for (int val : coinValues) {
        possibleSums |= (possibleSums << val);
    }

    vi finalSums;

    for (int i = 1; i <= 100000; ++i) {
        if (possibleSums[i]) finalSums.pb(i);
    }

    cout << sz(finalSums) << endl;
    
    forn(i, sz(finalSums)) {
        cout << finalSums[i] << (i == sz(finalSums) - 1 ? "" : " ");
    }
    cout << endl;
}
