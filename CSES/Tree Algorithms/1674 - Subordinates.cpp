...
void dfs(int boss, const vector<vll>& tree, vll& subordinates){
    for(int subordinate: tree[boss]){
        dfs(subordinate, tree, subordinates);
 
        subordinates[boss] += 1 +subordinates[subordinate];
    }
}
...
void solve() {
    ...
    dfs(0, tree, subordinates);

    for(ll i: subordinates){
        cout << i << " ";
    }
    cout << endl;
}
