...
void solve(){
    int queryCount;
    if(!(cin >> queryCount)) return;
    auto getJosephus = [&](auto self, ll n, ll k) -> ll {
        if(n == 1) return 1;
        if(k <= (n + 1) / 2){
            if(2 * k <= n) return 2 * k;
            return 1;
        }
        ll recursiveRes = self(self, n / 2, k - (n + 1) / 2);
        if(n % 2 == 0) return 2 * recursiveRes - 1;
        return 2 * recursiveRes + 1;
    };
    while(queryCount--){
        ll n, k;
        cin >> n >> k;
        cout << getJosephus(getJosephus, n, k) << endl;
    }
}
