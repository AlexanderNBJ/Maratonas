...
void solve() {
    ...
    cin >> gridSize >> numQueries;

    vector<vector<int>> prefixSum(gridSize + 1, vector<int>(gridSize + 1, 0));
    ...
    for1(i, gridSize) {
        ...
        for1(j, gridSize) {
            prefixSum[i][j] = (gridRow[j - 1] == '*') + prefixSum[i - 1][j] + prefixSum[i][j - 1] - prefixSum[i - 1][j - 1];
        }
    }
