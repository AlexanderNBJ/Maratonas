...
void solve(){
    ...
    cin >> n >> q;
    vll prices(n + 1);
    SegTree stMinus(n + 1), stPlus(n + 1);

    for1(i, n){
        cin >> prices[i];
        stMinus.update(i, prices[i] - i);
        stPlus.update(i, prices[i] + i);
    }
    ...
    forn(i, q){
        ...
        if(type == 1){
            ...
            cin >> k >> x;
            prices[k] = x;
            stMinus.update(k, x - k);
            stPlus.update(k, x + k);
        }else{
            int k;
            cin >> k;
            ll opt1 = stMinus.query(1, k) + k;
            ll opt2 = stPlus.query(k, n) - k;
