...
void solve() {
    ...
    BIT diffBit(numElements + 1);
    ll prevValue = 0;

    for1(i, numElements) {
        ll currentValue;
        cin >> currentValue;
        diffBit.update(i, currentValue - prevValue);
    ...
    forn(i, numQueries) {
        ...
        if (queryType == 1) {
            ...
            ll addValue;
            cin >> leftIdx >> rightIdx >> addValue;
            diffBit.update(leftIdx, addValue);
            diffBit.update(rightIdx + 1, -addValue);
        } else {
            int queryPos;
            cin >> queryPos;
            cout << diffBit.query(queryPos) << endl;
        }
    }
}
