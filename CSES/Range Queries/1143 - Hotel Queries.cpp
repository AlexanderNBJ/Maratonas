...
struct SegTree {
    ...
    int queryFirst(int node, int start, int end, ll val) {
        if (tree[node] < val) return -1;
        if (start == end) return start;
        int mid = (start + end) / 2;
        if (tree[2 * node] >= val) return queryFirst(2 * node, start, mid, val);
        return queryFirst(2 * node + 1, mid + 1, end, val);
    }

    void update(int idx, ll val) { update(1, 0, n - 1, idx, val); }
    int queryFirst(ll val) { return queryFirst(1, 0, n - 1, val); }
};
...
void solve() {
    ...
    SegTree st(n);
    vll hotelCapacities(n);

    forn(i, n) {
        cin >> hotelCapacities[i];
        st.update(i, hotelCapacities[i]);
    }
    ...
    forn(i, m) {
        ...
        int hotelIdx = st.queryFirst(groupSize);
        if (hotelIdx == -1) {
            cout << 0 << " ";
        } else {
            cout << hotelIdx + 1 << " ";
            hotelCapacities[hotelIdx] -= groupSize;
            st.update(hotelIdx, hotelCapacities[hotelIdx]);
        }
    }
    cout << endl;
}
