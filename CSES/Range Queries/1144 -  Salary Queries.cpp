...
void solve(){
    int n, q;
    cin >> n >> q;
    vi currentSalaries(n + 1);
    ordered_set<pii> salarySet;
    for1(i, n){
        cin >> currentSalaries[i];
        salarySet.insert({currentSalaries[i], i});
    }
    forn(i, q){
        char type;
        cin >> type;
        if(type == '!'){
            int k, x;
            cin >> k >> x;
            salarySet.erase({currentSalaries[k], k});
            currentSalaries[k] = x;
            salarySet.insert({currentSalaries[k], k});
        }else{
            int a, b;
            cin >> a >> b;
            int countUpToB = salarySet.order_of_key({b, INF});
            int countUpToA = salarySet.order_of_key({a, 0});
            cout << countUpToB - countUpToA << endl;
        }
    }
}
