...
void solve() {
    ...
    vector<Edge> edges;
    vector<vi> adj(numRooms + 1), revAdj(numRooms + 1);
    ...
    vector<bool> reachableFromStart(numRooms + 1, false);
    vector<bool> canReachEnd(numRooms + 1, false);

    auto bfs = [&](int startNode, vector<bool>& visited, vector<vi>& graph) {
        queue<int> q;
        q.push(startNode);
    ...
    bfs(1, reachableFromStart, adj);
    bfs(numRooms, canReachEnd, revAdj);
    vll maxScore(numRooms + 1, -INF64);
    maxScore[1] = 0;

    for1(i, numRooms - 1) {
        for (auto& edge : edges) {
            if (maxScore[edge.u] != -INF64) {
                if (maxScore[edge.u] + edge.weight > maxScore[edge.v]) {
                    maxScore[edge.v] = maxScore[edge.u] + edge.weight;
                }
            }
        }
    }
    ...
    for (auto& edge : edges) {
        if (maxScore[edge.u] != -INF64) {
            if (maxScore[edge.u] + edge.weight > maxScore[edge.v]) {
                if (reachableFromStart[edge.u] && canReachEnd[edge.v]) {
