...
void solve() {
    ...
    while (!q.empty()) {
        ...
        forn(i, 4) {
            int ni = curr.f + dx[i], nj = curr.s + dy[i];
            if (ni >= 0 && ni < n && nj >= 0 && nj < m && grid[ni][nj] != '#' && distM[ni][nj] == INF) {
                distM[ni][nj] = distM[curr.f][curr.s] + 1;
                q.push({ni, nj});
            }
        }
    }

    vector<vi> distA(n, vi(m, INF));
    ...
    while (!q.empty()) {
        ...
        if (curr.f == 0 || curr.f == n - 1 || curr.s == 0 || curr.s == m - 1) {
            exit_tile = curr;
            break;
        }
        ...
        forn(i, 4) {
            ...
            if (ni >= 0 && ni < n && nj >= 0 && nj < m && grid[ni][nj] == '.' && distA[ni][nj] == INF) {
                if (distA[curr.f][curr.s] + 1 < distM[ni][nj]) {
                    distA[ni][nj] = distA[curr.f][curr.s] + 1;
                    parent[ni][nj] = dir_chars[i];
                    q.push({ni, nj});
                }
            }
        }
