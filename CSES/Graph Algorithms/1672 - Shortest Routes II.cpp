...
void solve() {
    ...
    vector<vector<ll>> minDistance(numCities + 1, vector<ll>(numCities + 1, INF64));
    ...
    forn(i, numRoads) {
        ...
        ll roadWeight;
        cin >> u >> v >> roadWeight;
        minDistance[u][v] = min(minDistance[u][v], roadWeight);
        minDistance[v][u] = min(minDistance[v][u], roadWeight);
    }

    for1(k, numCities) {
        for1(i, numCities) {
            for1(j, numCities) {
                if (minDistance[i][k] != INF64 && minDistance[k][j] != INF64) {
    ...
    forn(i, numQueries) {
        int u, v;
        cin >> u >> v;
        if (minDistance[u][v] == INF64) {
            cout << -1 << endl;
        } else {
            cout << minDistance[u][v] << endl;
        }
    }
}
