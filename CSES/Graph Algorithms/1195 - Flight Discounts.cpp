...
void solve() {
    ...
    cin >> numCities >> numFlights;
    vector<vector<pair<int, ll>>> adjList(numCities + 1);

    forn(i, numFlights) {
        int cityU, cityV;
        ll flightPrice;
        cin >> cityU >> cityV >> flightPrice;
        adjList[cityU].pb({cityV, flightPrice});
    }

    vector<vector<ll>> minDistances(numCities + 1, vector<ll>(2, INF64));
    priority_queue<pair<ll, pii>, vector<pair<ll, pii>>, greater<pair<ll, pii>>> pq;
    minDistances[1][0] = 0;
    pq.push({0, {1, 0}});

    while (!pq.empty()) {
        ll currentCost = pq.top().f;
        int u = pq.top().s.f;
        int hasUsedDiscount = pq.top().s.s;
        pq.pop();

        if (currentCost > minDistances[u][hasUsedDiscount]) continue;

        for (auto& flight : adjList[u]) {
            int v = flight.f;
            ll price = flight.s;

            if (hasUsedDiscount == 0) {
                if (minDistances[v][0] > currentCost + price) {
                    minDistances[v][0] = currentCost + price;
                    pq.push({minDistances[v][0], {v, 0}});
                }
                if (minDistances[v][1] > currentCost + (price / 2)) {
                    minDistances[v][1] = currentCost + (price / 2);
                    pq.push({minDistances[v][1], {v, 1}});
                }
                
            } else {
                if (minDistances[v][1] > currentCost + price) {
                    minDistances[v][1] = currentCost + price;
                    pq.push({minDistances[v][1], {v, 1}});
                }
            }
        }
    }
    cout << minDistances[numCities][1] << endl;
}
