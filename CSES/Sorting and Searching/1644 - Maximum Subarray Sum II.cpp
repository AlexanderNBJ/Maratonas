...
void solve() {
    ...
    ll maxSubarraySum = -INF64;
    multiset<ll> validPrefixes;

    for (int j = a; j <= n; ++j) {
        validPrefixes.insert(prefixSum[j - a]);
        
        if (j > b) {
            validPrefixes.erase(validPrefixes.find(prefixSum[j - b - 1]));
        }

        maxSubarraySum = max(maxSubarraySum, prefixSum[j] - *validPrefixes.begin());
    }
