...
void solve() {
    ...
    multiset<int> memberFreeTimes;
    forn(i, k) {
        memberFreeTimes.insert(0);
    }

    int totalMovies = 0;

    forn(i, n) {
        int startTime = movies[i].s;
        int endTime = movies[i].f;

        auto it = memberFreeTimes.upper_bound(startTime);

        if (it != memberFreeTimes.begin()) {
            --it;
            memberFreeTimes.erase(it);
            memberFreeTimes.insert(endTime);
            totalMovies++;
        }
    }
